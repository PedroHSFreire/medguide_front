// components/views/pacient/HomePacient.tsx - VERSÃO CORRIGIDA
"use client";
...
export default function HomePacient() {
  ...
  const loadAllDoctors = async () => {
    try {
      ...
      // Busca médicos da API - com fallback para pré-cadastrados
      let doctors: DoctorSearchResult[] = [];
      try {
        doctors = await doctorSearchService.getAllDoctors();
      } catch (error) {
        console.warn(
          "API de médicos não disponível, usando médicos pré-cadastrados"
        );
      }
  ...
  const loadMyAppointments = async () => {
    ...
    try {
      ...
      console.warn("Não foi possível carregar consultas - lista vazia");
      // Em caso de erro, não carregar consultas de exemplo
      setMyAppointments([]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      // Busca médicos - com fallback para busca local
      let doctors: DoctorSearchResult[] = [];
      try {
        doctors = await doctorSearchService.searchDoctors({
          name: searchTerm,
          specialty: searchTerm,
        });
      } catch (error) {
        console.warn("Busca por API falhou, usando busca local");
      }
  ...
  const handleOpenAppointmentModal = async (doctor: DoctorSearchResult) => {
    ...
    try {
      // Buscar horários disponíveis - com fallback para horários de exemplo
      let slots: TimeSlot[] = [];
      try {
        slots = await appointmentService.getAvailableTimeSlots(
          doctor.id,
          today
        );
      } catch (error) {
        console.warn(
          "API de horários não disponível, gerando horários de exemplo"
        );
        slots = generateSampleTimeSlots();
      }
  ...
  const handleScheduleAppointment = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "API de agendamento não disponível, criando agendamento local"
        );
        // Fallback: criar agendamento localmente
        newAppointment = {
          id: `local-${Date.now()}`,
          ...appointmentData,
        ...
        updateUpcomingAppointments([...myAppointments, newAppointment]);
      }
      ...
      // Recarregar consultas se foi usado a API
      if (newAppointment.id && !newAppointment.id.startsWith("local-")) {
        await loadMyAppointments();
      }
    } catch (error) {
        ...
        {upcomingAppointments.length === 0 && (
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Você ainda não possui consultas agendadas. Encontre um médico
              abaixo e agende sua primeira consulta!
            </p>
          </div>
        )}
